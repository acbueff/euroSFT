"""
FRÓÐI Training Pipeline - SWEDISH EUROEVAL + SELF-PLAY ADAPTATION

This script orchestrates the complete FRÓÐI training pipeline for Swedish,
implementing a two-stage knowledge distillation followed by GRPO RL.
...
Phase 2: GRPO RL with EuroEval + Adaptive Self-Play
  - Uses Mistral-Small-3.1-24B as judge (best Swedish model on EuroEval)
  - Primary source: EuroEval Swedish training data
    (sentiment, NER, acceptability, reading comprehension, commonsense, knowledge)
  - Adaptive self-play: synthetic tasks from FineWeb corpus for mastered types
  - Mastery-weighted sampling: unmastered tasks get 3x weight, mastered get 1x
  - Three-way reward routing: deterministic, LLM+gold, LLM+example
"""
...
def run_grpo_euroeval(configs: dict, models: dict, kd_model_path: str = None, resume_checkpoint: str = None):
    """
    Run Phase 2: GRPO RL with EuroEval Training Data

    Uses Mistral-Small-3.1-24B as judge for Swedish tasks.
    Training source is exclusively EuroEval Swedish training data (sentiment, NER,
    acceptability, reading comprehension, commonsense, knowledge).
    Mastery-weighted sampling focuses on unmastered task types (3x weight).
    ...
    """
    logger.info("=" * 60)
    logger.info("PHASE 2: GRPO RL FOR SWEDISH (EUROEVAL)")
    logger.info("=" * 60)

    from src.core.frodi_trainer_swedish_euro import FrodiTrainerSwedish, load_euroeval_training_data
    ...
    # Load EuroEval Swedish training data
    euroeval_data_dir = configs['default'].get(
        'euroeval_train_dir',
        '/proj/berzelius-aiics-real/users/x_anbue/euroeval/train_sets/sv'
    )
    logger.info(f"Loading EuroEval training data from: {euroeval_data_dir}")
    euroeval_data = load_euroeval_training_data(euroeval_data_dir)
    total_samples = sum(len(samples) for samples in euroeval_data.values())
    logger.info(f"Loaded {len(euroeval_data)} EuroEval datasets with {total_samples} total samples")
    for ds_name, samples in euroeval_data.items():
        logger.info(f"  {ds_name}: {len(samples)} samples")

    # Initialize the Swedish GRPO trainer with EuroEval data
    trainer = FrodiTrainerSwedish(
        ...
        euroeval_data=euroeval_data
    )

    # Load corpora for optional pretraining loss (anti-forgetting LM objective only)
    corpora = load_swedish_corpora(configs['default'])

    # Run GRPO training (all tasks from EuroEval, corpora only for pretraining loss)
    ...
    logger.info("GRPO EuroEval training completed!")
    return save_path
...
def main():
    ...
    parser.add_argument('--phase', type=str, choices=['kd', 'grpo', 'all'], default='all',
                        help='Which phase to run: kd (knowledge distillation), grpo (EuroEval RL), or all')
    ...
    try:
        ...
        if args.phase in ['grpo', 'all']:
            run_grpo_euroeval(configs, models, kd_model_path, resume_checkpoint=args.resume_from)
