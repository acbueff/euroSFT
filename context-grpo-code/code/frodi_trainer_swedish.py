"""
FRÓÐI Trainer - SWEDISH ADAPTATION

Implements a two-stage knowledge distillation process for Swedish language acquisition:
- Stage 1 (CPT): Continued Pre-Training for Swedish fluency
- Stage 2 (SFT): Supervised Fine-Tuning for reasoning in Swedish

Followed by GRPO self-play RL with Swedish-optimized reward signals.

ANTI-FORGETTING MEASURES:
- Replay buffer with pretraining data during SFT
- Lower learning rate with warmup
- Diverse task coverage (QA, MMLU-style, reasoning)
...
class FrodiTrainerSwedish:
    """
    FRÓÐI Trainer for Swedish - GRPO Self-Play RL Phase
    
    Uses the distilled student model and Mistral-Small-3.1-24B as judge.
    """
    ...
    def __init__(
        ...
        # Initialize Swedish reward model
        from ..rl_loop.reward_swedish import TeacherStudentSimpleRewardSwedish
    ...
    def _build_swedish_system_prompts(self) -> Dict[str, Dict[str, str]]:
        """Build Swedish-specific system prompts"""
        prompts_cfg = self.config.get('prompts', {})
        
        return {
            'translation': {
                'student': prompts_cfg.get('system_translation') or 
                    "Du är en översättare. Översätt användarens text korrekt. Ge endast översättningen utan förklaringar.",
                'teacher': "Du är en precis svensk↔engelsk översättare. Översätt texten korrekt och ge endast översättningen.",
            },
            'qa_sv': {
                'student': prompts_cfg.get('system_qa_sv') or
                    "Du är en precis fråga-svar-assistent. Besvara frågor endast baserat på den givna kontexten och svara tydligt på svenska.",
                'teacher': "Du är en noggrann QA-bedömare. Besvara frågan endast med information från kontexten.",
            },
            'qa_en': {
                'student': prompts_cfg.get('system_qa_en') or
                    "You are a precise question answering assistant. Answer using only the provided context.",
                'teacher': "You are a meticulous QA judge. Answer using only information from the context.",
            },
            'summary': {
                'student': prompts_cfg.get('system_summary') or
                    "Du är en precis sammanfattare. Sammanfatta texten i två till tre meningar.",
                'teacher': "Du är en noggrann sammanfattningsbedömare.",
            },
        }
    ...
    def _get_active_weights(self, weights: Dict[str, float]) -> Dict[str, float]:
        ...
        return {k: float(v) for k, v in (weights or {}).items() if float(v) > 0.0}
    ...
    def train_self_play_rl(
        ...
        """Run GRPO self-play RL training for Swedish with periodic EuroEval evaluations
        
        Args:
            monolingual_corpora: Dictionary of monolingual corpora by language
            n_iterations: Total number of iterations to train
            start_iteration: Iteration to resume from (0 to start fresh)
        """
        logger.info("Starting Swedish GRPO Self-Play RL Training")
        ...
        for iteration in range(actual_start, n_iterations + 1):
            ...
            self._populate_task_buffer(monolingual_corpora)
            if iteration <= 5:
            ...
            if iteration <= 5:
                logger.info(f"  [Iter {iteration}] Starting self-play rollout...")
            ...
            if iteration % grad_accum == 0:
                ...
                avg_reward = float(np.mean([
                    sum(active_weights.get(k, 0.0) * v for k, v in r.items()) for r in rewards
                ]))
            ...
            if iteration % log_interval == 0:
                ...
                if hasattr(self, 'last_avg_kl'):
                    ...
                    if self.last_avg_kl < -2.0:
                        ...
                        logger.warning(
                            f"⚠️ NEGATIVE KL DETECTED ({self.last_avg_kl:.2f}) - "
                        ...
                        if self.negative_kl_warnings >= 5:
                            logger.warning(
                                "⚠️ REPEATED NEGATIVE KL - Consider increasing kl_coef or "
    ...
    def _log_training_metrics(self, iteration: int, loss: float, reward: float):
        """Log training metrics to CSV for tracking progress."""
    ...
    def _populate_task_buffer(self, corpora: Dict[str, Dataset]):
        """Populate task buffer with Swedish tasks"""
        min_tasks = self.config.get('rl', {}).get('batch_size', 8) * 2
        max_attempts = min_tasks * 10  # Prevent infinite loop
        attempts = 0
        
        while self.task_buffer.size < min_tasks and attempts < max_attempts:
            task = self._generate_swedish_task(corpora)
            if task:
                self.task_buffer.insert(task)
            attempts += 1
        
        if self.task_buffer.size < min_tasks:
            logger.warning(
                f"Task buffer under-populated after {attempts} attempts: "
                f"{self.task_buffer.size}/{min_tasks} tasks"
            )
    
    def _generate_swedish_task(self, corpora: Dict[str, Dataset]) -> Optional[Task]:
        """Generate a Swedish task"""
        task_types = self.config.get('curriculum', {}).get('task_types', {
            'translation': 0.5, 'qa': 0.4, 'summary': 0.1
        })
        ...
        if task_type == 'translation':
            return self._generate_translation_task(corpora)
        elif task_type == 'qa':
            return self._generate_qa_task(corpora)
        elif task_type == 'summary':
            return self._generate_summary_task(corpora)
        
        return None
    
    def _generate_translation_task(self, corpora: Dict[str, Dataset]) -> Optional[Task]:
        """Generate Swedish↔English translation task"""
        source_lang = random.choice(['sv', 'en'])
        target_lang = 'en' if source_lang == 'sv' else 'sv'
        
        if source_lang not in corpora:
            return None
        
        corpus = corpora.get(source_lang)
        if corpus is None:
            return None
        
        # Efficiently sample from large corpus without loading all texts into memory
        try:
            corpus_len = len(corpus)
            if corpus_len == 0:
                return None
            random_idx = random.randint(0, corpus_len - 1)
            source_text = corpus[random_idx]['text']
        except Exception:
            # Fallback to extraction method for non-Dataset objects
            texts = _extract_texts_from_corpus(corpus)
            if not texts:
                return None
    ...
    def _generate_qa_task(self, corpora: Dict[str, Dataset]) -> Optional[Task]:
        """Generate Swedish QA task"""
        corpus = corpora.get('sv')
        if corpus is None:
            return None
        
        # Efficiently sample from large corpus
        try:
            corpus_len = len(corpus)
            if corpus_len == 0:
                return None
            random_idx = random.randint(0, corpus_len - 1)
            context = corpus[random_idx]['text']
        except Exception:
            texts = _extract_texts_from_corpus(corpus)
            if not texts:
                return None
        ...
        prompt = f"Kontext: {context[:500]}\n\nFråga: Vad handlar texten om?\n\nSvar:"
    ...
    def _generate_summary_task(self, corpora: Dict[str, Dataset]) -> Optional[Task]:
        """Generate Swedish summary task"""
        corpus = corpora.get('sv')
        if corpus is None:
            return None
        ...
        try:
            ...
            if corpus_len == 0:
                return None
            random_idx = random.randint(0, corpus_len - 1)
            text = corpus[random_idx]['text']
        except Exception:
            texts = _extract_texts_from_corpus(corpus)
            if not texts:
                return None
            text = random.choice(texts)
        prompt = f"Sammanfatta följande text på svenska:\n\n{text[:600]}"
    ...
    def _self_play_rollout(self, task_batch: List[Task]) -> Tuple[List[str], List[Dict], List[float]]:
        ...
        for task_idx, task in enumerate(task_batch):
            ...
            # Update task buffer with results
            for sol, reward_dict in zip(solutions, rewards):
                active_weights = self._get_active_weights(self.weight_scheduler.get_weights())
                total_reward = sum(active_weights.get(k, 0.0) * v for k, v in reward_dict.items())
                success = total_reward > 0.0  # Simple success criterion
